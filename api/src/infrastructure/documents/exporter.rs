...
impl DocumentExportAssets {
    ...
    fn attachments(&self) -> &[DocumentExportAttachment] {
        &self.attachments
    }

    fn markdown_string(&self) -> anyhow::Result<String> {
        String::from_utf8(self.markdown.clone())
...
fn build_archive(assets: &DocumentExportAssets) -> anyhow::Result<Vec<u8>> {
    ...
    let mut cursor = std::io::Cursor::new(Vec::new());
    {
        ...
        for attachment in assets.attachments() {
            let entry = format!("{}/{}", assets.safe_title, attachment.trimmed_path());
