...
pub(super) fn ensure_share_create_permission(permissions: &PermissionSet) -> Result<(), ServiceError> {
    sharing_policy::ensure_share_create_allowed(permissions).map_err(|_| ServiceError::Forbidden)
}

pub(super) fn ensure_share_delete_permission(permissions: &PermissionSet) -> Result<(), ServiceError> {
