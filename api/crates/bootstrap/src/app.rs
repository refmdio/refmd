...
use application::ports::plugin_event_publisher::PluginEventPublisher;
...
use application::services::doc_events::{
...
use application::services::plugins::asset_signer::AssetSigner;
...
use application::services::realtime::snapshot::MarkdownExportProvider;
use application::services::shares::ShareService;
use application::services::storage_ingest::StorageIngestService;
...
use application::services::workspaces::{WorkspacePermissionResolver, WorkspaceService};
use crate::auth;
use crate::config::{Config, StorageBackend};
use crate::git::{self, GitStack};
use crate::jobs::{self, Jobs};
use crate::{http, plugins, realtime, telemetry};
...
pub struct AppRuntime {
    ...
    pool: PgPool,
    ctx: AppContext,
    local_hub: Option<infrastructure::realtime::Hub>,
    jobs: Jobs,
...
impl AppRuntime {
    ...
    pub fn into_parts(
        ...
        PgPool,
        AppContext,
        Option<infrastructure::realtime::Hub>,
        Jobs,
...
pub async fn build_runtime(
    ...
    let snapshot_archive_repo: Arc<
        dyn application::ports::document_snapshot_archive_repository::DocumentSnapshotArchiveRepository,
    > = Arc::new(
        infrastructure::db::repositories::document_snapshot_archive_repository_sqlx::SqlxDocumentSnapshotArchiveRepository::new(
            pool.clone(),
        ),
    );
    let document_repo = Arc::new(
        infrastructure::db::repositories::document_repository_sqlx::SqlxDocumentRepository::new(
            pool.clone(),
        ),
    );
    ...
    let shares_repo_impl = Arc::new(
        infrastructure::db::repositories::shares_repository_sqlx::SqlxSharesRepository::new(
            pool.clone(),
        ),
    );
    let share_service = Arc::new(ShareService::new(shares_repo_impl.clone()));
    let access_repo = Arc::new(
        infrastructure::db::repositories::access_repository_sqlx::SqlxAccessRepository::new(
            pool.clone(),
        ),
    );
    ...
    let files_repo = Arc::new(
        infrastructure::db::repositories::files_repository_sqlx::SqlxFilesRepository::new(
            pool.clone(),
        ),
    );
    let public_repo = Arc::new(
        infrastructure::db::repositories::public_repository_sqlx::SqlxPublicRepository::new(
            pool.clone(),
        ),
    );
    let user_repo = Arc::new(
        infrastructure::db::repositories::user_repository_sqlx::SqlxUserRepository::new(
            pool.clone(),
        ),
    );
    let workspace_repo = Arc::new(
        infrastructure::db::repositories::workspace_repository_sqlx::SqlxWorkspaceRepository::new(
            pool.clone(),
        ),
    );
    ...
    let tag_repo = Arc::new(
        infrastructure::db::repositories::tag_repository_sqlx::SqlxTagRepository::new(
            pool.clone(),
        ),
    );
    let tag_service = Arc::new(TagService::new(tag_repo.clone()));
    let api_token_repo = Arc::new(
        infrastructure::db::repositories::api_token_repository_sqlx::SqlxApiTokenRepository::new(
            pool.clone(),
        ),
    );
    ...
    let user_session_repo = Arc::new(
        infrastructure::db::repositories::user_session_repository_sqlx::SqlxUserSessionRepository::new(
            pool.clone(),
        ),
    );
    ...
    let user_shortcuts = Arc::new(
        infrastructure::db::repositories::user_shortcut_repository_sqlx::SqlxUserShortcutRepository::new(
            pool.clone(),
        ),
    );
    ...
    let recent_projection_cache = Arc::new(
        application::services::storage_projection_cache::RecentProjectionCache::new(
    ...
    let plugin_repo = Arc::new(
        infrastructure::db::repositories::plugin_repository_sqlx::SqlxPluginRepository::new(
            pool.clone(),
        ),
    );
    let plugin_data_service = Arc::new(PluginDataService::new(plugin_repo.clone()));
    let plugin_installations = Arc::new(
        infrastructure::db::repositories::plugin_installation_repository_sqlx::SqlxPluginInstallationRepository::new(
            pool.clone(),
        ),
    );
    ...
    let plugin_event_bus = Arc::new(
        infrastructure::plugins::event_bus_pg::PgPluginEventBus::new(
            pool.clone(),
            "plugin_events",
        ),
    );
    ...
    let health_probe =
        infrastructure::health::db_probe::DatabaseHealthProbe::new(pool.clone());
