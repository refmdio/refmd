...
impl SharesRepository for SqlxSharesRepository {
    async fn create_share(
        ...
        let out: anyhow::Result<CreatedShare> = async {
            // Verify ownership and type
            let dtype_raw: String = sqlx::query_scalar(
                "SELECT type FROM documents WHERE id = $1 AND workspace_id = $2",
            )
            .bind(document_id)
            .bind(workspace_id)
            .fetch_optional(&self.pool)
    ...
    async fn list_share_mounts(&self, workspace_id: Uuid) -> PortResult<Vec<ShareMountRow>> {
        let out: anyhow::Result<Vec<ShareMountRow>> = async {
    ...
    async fn create_share_mount(
        ...
        let out: anyhow::Result<ShareMountRow> = async {
    ...
    async fn list_applicable_shares_for_doc(
        ...
        let out: anyhow::Result<Vec<ApplicableShareRow>> = async {
            let rows = sqlx::query(
    ...
    async fn list_subtree_nodes(&self, root_id: Uuid) -> PortResult<Vec<ShareSubtreeNode>> {
        let out: anyhow::Result<Vec<ShareSubtreeNode>> = async {
            let rows = sqlx::query(
                r#"
            WITH RECURSIVE subtree AS (
...
impl ShareAccessPort for SqlxSharesRepository {
    ...
    async fn get_materialized_permission(
        ...
        doc_id: Uuid,
    ) -> PortResult<Option<share::SharePermission>> {
        let out: anyhow::Result<Option<share::SharePermission>> = async {
