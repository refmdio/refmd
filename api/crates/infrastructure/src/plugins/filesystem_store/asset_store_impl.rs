...
impl PluginAssetStore for FilesystemPluginStore {
    async fn fetch_asset(
        ...
        version: &str,
        relative_path: &str,
    ) -> application::core::ports::errors::PortResult<PluginAssetPayload> {
    ...
    async fn remove_user_plugin_dir(
        &self,
        user_id: &Uuid,
        plugin_id: &str,
    ) -> application::core::ports::errors::PortResult<()> {
        let out: anyhow::Result<()> =
            async { FilesystemPluginStore::remove_user_plugin_dir(self, user_id, plugin_id) }
                .await;
        out.map_err(Into::into)
    }
    ...
    async fn list_latest_global_manifests(
        &self,
    ) -> application::core::ports::errors::PortResult<Vec<LatestGlobalManifest>> {
    ...
    async fn load_user_manifest(
        ...
        plugin_id: &str,
        version: &str,
    ) -> application::core::ports::errors::PortResult<Option<serde_json::Value>> {
