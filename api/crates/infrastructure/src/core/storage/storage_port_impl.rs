...
impl StorageProjectionPort for FsStoragePort {
    async fn move_folder_subtree(&self, folder_id: Uuid) -> PortResult<usize> {
        let out: anyhow::Result<usize> = async {
            crate::core::storage::move_folder_subtree(
                &self.pool,
    ...
    async fn delete_folder_physical(&self, folder_id: Uuid) -> PortResult<usize> {
        let out: anyhow::Result<usize> = async {
            crate::core::storage::delete_folder_physical(
                &self.pool,
                self.uploads_root.as_path(),
                folder_id,
            )
    ...
    async fn sync_doc_paths(&self, doc_id: Uuid) -> PortResult<()> {
        let out: anyhow::Result<()> = async {
            crate::core::storage::move_doc_paths(&self.pool, self.uploads_root.as_path(), doc_id)
                .await
        }
        .await;
        out.map_err(Into::into)
...
impl StorageResolverPort for FsStoragePort {
    ...
    fn relative_from_uploads(&self, abs: &Path) -> String {
        crate::core::storage::relative_from_uploads(self.uploads_root.as_path(), abs)
    }
    ...
    async fn exists(&self, abs_path: &Path) -> PortResult<bool> {
        let out: anyhow::Result<bool> =
            async { Ok(tokio::fs::try_exists(abs_path).await.unwrap_or(false)) }.await;
        out.map_err(Into::into)
    }
    ...
    async fn write_bytes(&self, abs_path: &Path, data: &[u8]) -> PortResult<()> {
        let out: anyhow::Result<()> = async {
            ...
            let is_text = abs_path
                ...
                .unwrap_or(false);
            let _ = crate::core::storage::mark_dirty_upsert_abs_path(
                &self.pool,
                self.uploads_root.as_path(),
                abs_path,
                is_text,
