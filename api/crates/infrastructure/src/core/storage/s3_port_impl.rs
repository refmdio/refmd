        ...
        if !files.is_empty() {
            ...
            for row in files {
                ...
                let new_rel_attachment =
                    crate::core::storage::relative_from_uploads(&self.root, &new_path)
                        .replace('\\', "/");
...
impl StorageProjectionPort for S3StoragePort {
    ...
    async fn sync_doc_paths(&self, doc_id: Uuid) -> PortResult<()> {
        let out: anyhow::Result<()> = async { self.move_doc_paths(doc_id).await }.await;
        out.map_err(Into::into)
    }
    ...
    async fn delete_relative_path(&self, rel: &str) -> PortResult<()> {
        let out: anyhow::Result<()> = async {
            ...
            let _ = self.delete_object(&key).await;
            crate::core::storage::mark_dirty_delete_relative(&self.pool, rel).await?;
            Ok(())
        }
        .await;
        out.map_err(Into::into)
...
impl StorageResolverPort for S3StoragePort {
    async fn build_doc_dir(&self, doc_id: Uuid) -> PortResult<PathBuf> {
        let out: anyhow::Result<PathBuf> =
            async { crate::core::storage::build_doc_dir(&self.pool, &self.root, doc_id).await }
                .await;
        out.map_err(Into::into)
    }

    async fn build_doc_file_path(&self, doc_id: Uuid) -> PortResult<PathBuf> {
        let out: anyhow::Result<PathBuf> = async {
            crate::core::storage::build_doc_file_path(&self.pool, &self.root, doc_id).await
        }
        .await;
        out.map_err(Into::into)
    }
    ...
    fn relative_from_uploads(&self, abs: &Path) -> String {
        crate::core::storage::relative_from_uploads(&self.root, abs).replace('\\', "/")
    }
    ...
    async fn resolve_upload_path(&self, doc_id: Uuid, rest_path: &str) -> PortResult<PathBuf> {
        let out: anyhow::Result<PathBuf> = async {
            let doc_dir = crate::core::storage::build_doc_dir(&self.pool, &self.root, doc_id)
                .await?
                .to_path_buf();
    ...
    async fn write_bytes(&self, abs_path: &Path, data: &[u8]) -> PortResult<()> {
        let out: anyhow::Result<()> = async {
            ...
            let rel = crate::core::storage::relative_from_uploads(&self.root, abs_path)
                .replace('\\', "/");
            let _ = crate::core::storage::mark_dirty_upsert_relative(&self.pool, &rel, true, None)
                .await;
            Ok(())
        }
        .await;
    ...
    async fn store_doc_attachment(
        ...
        let out: anyhow::Result<StoredAttachment> = async {
            use tokio::fs;
