...
impl StorageProjectionPort for FsStoragePort {
    async fn move_folder_subtree(&self, folder_id: Uuid) -> anyhow::Result<usize> {
        crate::storage::move_folder_subtree(
            &self.pool,
    ...
    async fn delete_doc_physical(&self, doc_id: Uuid) -> anyhow::Result<()> {
        crate::storage::delete_doc_physical(
            &self.pool,
            self.uploads_root.as_path(),
            doc_id,
        )
    ...
    async fn delete_folder_physical(&self, folder_id: Uuid) -> anyhow::Result<usize> {
        crate::storage::delete_folder_physical(
            &self.pool,
            self.uploads_root.as_path(),
            folder_id,
        )
...
impl StorageResolverPort for FsStoragePort {
    ...
    async fn build_doc_file_path(&self, doc_id: Uuid) -> anyhow::Result<PathBuf> {
        crate::storage::build_doc_file_path(
            &self.pool,
            self.uploads_root.as_path(),
            doc_id,
        )
    ...
    fn relative_from_uploads(&self, abs: &Path) -> String {
        crate::storage::relative_from_uploads(self.uploads_root.as_path(), abs)
    }
    ...
    async fn resolve_upload_path(&self, doc_id: Uuid, rest_path: &str) -> anyhow::Result<PathBuf> {
        ...
        let doc_dir = crate::storage::build_doc_dir(
            &self.pool,
            self.uploads_root.as_path(),
            doc_id,
        )
    ...
    async fn write_bytes(&self, abs_path: &Path, data: &[u8]) -> anyhow::Result<()> {
        ...
        let is_text = abs_path
            ...
            .unwrap_or(false);
        let _ = crate::storage::mark_dirty_upsert_abs_path(
            &self.pool,
            self.uploads_root.as_path(),
            abs_path,
            is_text,
    ...
    async fn store_doc_attachment(
        ...
        let base_dir = crate::storage::build_doc_dir(
            &self.pool,
            self.uploads_root.as_path(),
            doc_id,
        )
        ...
        let mut safe = crate::storage::sanitize_title(original);
        ...
        let relative = crate::storage::relative_from_uploads(
            self.uploads_root.as_path(),
            &candidate,
        )
        .replace('\\', "/");
