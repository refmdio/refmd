...
export async function createDeviceRevocationAuthorization(params: {
  worker: Worker;
  ...
  if (scopes.length !== 1 || scopes[0]?.chain_scope_kind !== "user") {
    throw new Error("device_revocation_intent_scope_invalid");
  }
  const scope = scopes[0]!;
  const event = records(scope.candidate_events).at(-1);
  const firstEffect = records(scope.candidate_key_directory_effects)[0];
  if (!event || !firstEffect) throw new Error("device_revocation_intent_incomplete");
  ...
  const checkpointPayload: StrictJsonValue = {
    ...
    signing_key_id: signerKeyIdForRequirement(scope, "device_revocation"),
    authorization_checkpoint_scope_kind: "user",
    authorization_checkpoint_scope_id: string(scope.chain_scope_id),
    authorization_checkpoint_sequence: integer(kdActor.key_checkpoint_sequence),
    authorization_checkpoint_hash: string(kdActor.key_checkpoint_hash),
  ...
  for (const requirement of records(scope.effect_signature_requirements)) {
    ...
    if (kind === "key_directory_event") {
      ...
      if (!effect) throw new Error("device_revocation_effect_missing");
      const payload = record(effect.event_payload);
      artifact = await params.worker.signIdentityKeyDirectoryEvent({
        eventType: string(payload.event_type),
        eventPayload: payload,
      });
    } else if (kind === "key_directory_checkpoint") {
      artifact = await params.worker.signIdentityKeyDirectoryCheckpoint({
        variant: "identity_active",
        checkpointPayload: record(scope.candidate_key_directory_checkpoint_payload),
      });
    } else if (kind === "device_revocation") {
      const target = revocationTarget(effects, event, string(scope.chain_scope_id));
      artifact = await params.worker.createDeviceRevocationSignature({
        actor: {
          user_id: string(auditActor.user_id),
          device_id: string(auditActor.device_id),
          signing_key_id: string(requirement.signer_key_id),
          key_scope_kind: "user",
          key_scope_id: string(scope.chain_scope_id),
          key_checkpoint_sequence: integer(kdActor.key_checkpoint_sequence),
          key_checkpoint_hash: string(kdActor.key_checkpoint_hash),
        },
        revokedDevice: target,
        authorityBoundary: {
          revocation_event_sequence: integer(event.sequence),
          revocation_event_hash: string(event.event_hash),
        },
      });
    } else {
      throw new Error("device_revocation_authorization_kind_invalid");
    }
...
export function materializeDeviceRevocationKeyDirectory(
  ...
  if (!checkpoint) throw new Error("device_revocation_checkpoint_missing");
...
function revocationTarget(
  ...
  const signing = record(record(effects[0]!.event_payload).body);
  const encryption = record(record(effects[1]!.event_payload).body);
  return {
    user_id: userId,
    device_id: string(record(event.event_body).subject_id),
    signing_key_id: string(signing.key_id),
    encryption_key_id: string(encryption.key_id),
  };
}

function signerKeyIdForRequirement(scope: Record<string, unknown>, kind: string): string {
  const requirement = records(scope.effect_signature_requirements).find(
    (value) => value.authorization_kind === kind,
  );
  if (!requirement) throw new Error("device_revocation_requirement_missing");
  return string(requirement.signer_key_id);
}
...
function literal(actual: unknown, expected: unknown): void {
  if (actual !== expected) throw new Error("device_revocation_binding_invalid");
}
