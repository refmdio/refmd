...
import { buildMountChildRows, type SidebarTreeNode } from "../model/rows";
...
function mountTitle(mount: ShareMount): string {
  return (
    mount.title ??
    mount.target.title ??
    (mount.target_kind === "folder" ? "Shared folder" : "Shared document")
  );
}

function mountKey(mount: ShareMount): string {
  return `${mount.id}:${mount.target_document_id}`;
}

function entryKey(mount: ShareMount, entry: ShareTreeEntry): string {
  return `${mount.id}:${entry.id}`;
}
