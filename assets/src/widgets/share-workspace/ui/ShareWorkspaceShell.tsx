...
export function ShareWorkspaceShell(props: ShareWorkspaceShellProps) {
  ...
  const existingMounts = useShareMountLookup({
    shareSlug: () => props.shareSlug,
    enabled: userSessionAvailable,
    initialMounts: () => props.existingMounts,
  });
  ...
  const openDocumentToken = async (documentToken: string) => {
    ...
    try {
      ...
      const target = createRawShareDocumentPanelTarget(resolved.target);
  ...
  createEffect(() => {
    for (const stateKey of registeredTargetKeys()) {
      ...
      navigate(`/share/${state.access.shareSlug}`, {
        replace: true,
        scroll: false,
      });
      return;
          ...
          <For each={tree()}>
            {(node) => (
              <ShareTreeItem
                ...
                existingMounts={existingMounts.mounts()}
                sessionAvailable={userSessionAvailable()}
                shareSlug={props.shareSlug}
                onMountSaved={() => {
                  if (props.existingMounts == null) void existingMounts.refetch();
                }}
                onToggle={toggle}
                onOpen={openEntry}
              />
