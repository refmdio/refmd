...
export function createDocumentSyncFailure(
  ...
  const fallbackMessage = "Unknown document sync error";
  const message = error instanceof Error ? error.message : reason || fallbackMessage;
  const reasonMessage = reason || message || fallbackMessage;
