...
export function handleEphemeralMessage(
  ...
  processEphemeral(payload, state, documentId, localDeviceSigningKeyId).catch((err) => {
    clientWarn("ephemeral_processing_failed", { documentId, error: err });
  });
}
async function processEphemeral(
  ...
  const senderSigningKeyId = pd.signingKeyId;
  if (!senderSigningKeyId || !pd) return;
  if (pd.docId !== documentId) return;
  const publicData = pd as unknown as Record<string, unknown>;
  // Same device (same signingKeyId): skip. This covers both own broadcasts
  ...
  if (senderSigningKeyId === localDeviceSigningKeyId) return;
  const resolveResult = await resolveSigningKey(senderSigningKeyId, state);
  ...
  const valid = await worker.verifyEditorEphemeralSignature({
    ...
    actorUserId: resolveResult.ownerId,
  ...
  const result = await handleIncomingEphemeral(
    ...
    senderSigningKeyId,
    resolveResult.key,
    resolveResult.ownerId,
    worker,
    state.access.kind === "share" ? "share_participant_device" : "device",
    {
      workspaceId: state.workspaceId,
      documentId,
  ...
  switch (result.action) {
    case "respond":
      ...
      if (state.ephemeralSession?.trustedPeers.has(remoteSessionIdB64)) {
        await resendAwareness(state, documentId, localDeviceSigningKeyId!);
      }
      break;
    case "trusted":
      await resendAwareness(state, documentId, localDeviceSigningKeyId!);
    ...
    case "awareness": {
      ...
      for (const [clientId, clientState] of states) {
        ...
        if (existingOwner && existingOwner !== senderSigningKeyId) {
      ...
      for (const clientId of removedClients) {
        ...
        if (owner && owner !== senderSigningKeyId) {
      ...
      // If a clientID was previously owned by a different signingKeyId, restore
      ...
      for (const clientId of changedClients) {
        ...
        if (existingOwner && existingOwner !== senderSigningKeyId) {
          ...
          state.awarenessClientOwners.set(clientId, senderSigningKeyId);
        }
      }
      ...
      // Dedup: same signingKeyId should have at most one clientID (per-device granularity).
      ...
      for (const clientId of changedClients) {
        for (const [existingId, owner] of state.awarenessClientOwners) {
          if (owner === senderSigningKeyId && existingId !== clientId) {
      ...
      const verifiedUserId = state.signingKeyOwners.get(senderSigningKeyId);
      if (verifiedUserId) {
        ...
        for (const clientId of changedClients) {
          ...
          if (awarenessState?.user) {
            ...
            if (awarenessState.user.signingKeyId !== senderSigningKeyId) {
              awarenessState.user.signingKeyId = senderSigningKeyId;
              needsReemit = true;
            }
          }
        }
...
export function handlePeerLeft(payload: PeerLeftPayload, state: DocumentState): void {
  ...
  state.awareness.getStates().forEach((awarenessState, clientId) => {
    ...
    const user = awarenessState.user as
      ...
      | undefined;
    if (user?.signingKeyId === signingKeyId) {
