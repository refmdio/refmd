...
export async function doInitializeDocumentSync(
  ...
  let localDeviceSigningPubKey: string | undefined;
  ...
  for (let attempt = 0; ; attempt += 1) {
    try {
      ...
      localDeviceSigningPubKey = prepared.localDeviceSigningPubKey;
  ...
  if (!documentPayload || !failClosed || !deviceKeyCachePromise || !oldDekPrimePromise) return;
  assertActive();
  ...
  if ("error" in deviceKeyCacheOutcome) {
    ...
    throw failClosedForInitialLoad(failClosed, deviceKeyCacheOutcome.error);
  }
  ...
  if (!state.readOnly && localDeviceSigningPubKey) {
    ...
    const writerLock = acquireDocumentWriterLock(documentId, localDeviceSigningPubKey, state, {
