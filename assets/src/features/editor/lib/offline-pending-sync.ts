...
import { DocumentChannelError } from "./document-sync-error";
...
// Re-use the shared cancellation check from document-sync-init
import { isInitCancelledError as isCancelledSyncError } from "./document-sync-init";
...
function getAccessDeniedReason(error: unknown): PendingSyncBlockReason | null {
  if (error instanceof DocumentChannelError && isAccessDeniedReason(error.code)) {
    return error.code;
  }
