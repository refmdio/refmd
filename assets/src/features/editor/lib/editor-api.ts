...
// --- Offset ↔ Position helpers for plain text ---
...
export class EditorApi implements EditorLike {
  ...
  transaction(tx: EditorTransaction): void {
    ...
    if (tx.changes && tx.changes.length > 0) {
      specs.changes = tx.changes.map((c) => ({
        from: this.posToOffset(c.from),
        to: c.to ? this.posToOffset(c.to) : undefined,
        insert: c.text,
      }));
    }
    if (tx.selection) {
  ...
  wordAt(pos: EditorPosition): EditorRange | null {
    const offset = this.posToOffset(pos);
    const word = this.cm.state.wordAt(offset);
    if (!word) return null;
    return { from: this.offsetToPos(word.from), to: this.offsetToPos(word.to) };
  }
}

// --- ProseMirror EditorApi ---
    ...
    if (tx.changes && tx.changes.length > 0 && this.yText) {
      ...
      this.yText.doc!.transact(() => {
        ...
        for (const c of tx.changes!) {
          const from = positionToOffset(snapshot, c.from) + offsetShift;
          const to = c.to ? positionToOffset(snapshot, c.to) + offsetShift : from;
          if (to > from) this.yText!.delete(from, to - from);
          if (c.text) this.yText!.insert(from, c.text);
          offsetShift += c.text.length - (to - from);
        }
      });
      ...
      for (const c of tx.changes) {
        ...
        const to = c.to ? this.posToOffset(c.to) : from;
        tr.replaceWith(from, to, this.pm.state.schema.text(c.text));
      }
      this.pm.dispatch(tr);
    }
    if (tx.selection) {
