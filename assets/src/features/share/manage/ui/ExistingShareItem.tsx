...
import type { ShareListItem } from "../model/types";
import type { ShareManagementModel } from "../model/useShareManagement";
import {
  buildDescendantRows,
  OptionSelect,
...
export function ExistingShareItem(props: { share: ShareListItem; state: ShareManagementModel }) {
  ...
  const [accessLimit, setAccessLimit] = createSignal(props.share.access_limit?.toString() ?? "");
  ...
  const hasSettingsChanges = () =>
    expiryDays() !== "" || accessLimit() !== (props.share.access_limit?.toString() ?? "");
      ...
      <div class="flex items-center justify-between gap-2">
        <div>
          ...
          <div class="text-xs text-muted-foreground">
            ...
            {props.share.access_count}
            {props.share.access_limit == null ? "" : `/${props.share.access_limit}`} uses
            {props.share.expires_at
              ? ` · Expires ${new Date(props.share.expires_at).toLocaleDateString()}`
              : ""}
          </div>
        </div>
      </div>
