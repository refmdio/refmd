defmodule RefMD.Sharing.Access do
  ...
  def share_accepting_participants?(%Share{} = share) do
    not expired?(share) and not access_limit_reached?(share)
  end

  @spec share_session_accessible?(Share.t()) :: boolean()
  def share_session_accessible?(%Share{} = share) do
    # access_limit is enforced only when admitting a new participant session.
    # Once admitted, the same session may continue canonical bootstrap,
    # reload, reconnect, and channel revalidation until the share expires.
    not expired?(share)
  end
  ...
  def expired?(%Share{expires_at: nil}), do: false

  def expired?(%Share{expires_at: expires_at}) do
    DateTime.compare(expires_at, DateTime.utc_now()) != :gt
  end

  @spec access_limit_reached?(Share.t()) :: boolean()
  def access_limit_reached?(%Share{access_limit: nil}), do: false

  def access_limit_reached?(%Share{access_limit: access_limit, access_count: access_count}) do
  ...
  defp active_participant_share_for_document(share_id, document_id, session_id) do
    with %{access_share: %Share{} = access_share} = payload <- find_share_access_payload(share_id),
         ...
         true <- session.share_id == access_share.id,
