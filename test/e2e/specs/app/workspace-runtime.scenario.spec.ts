...
test.describe.serial("Workspace Runtime", () => {
  ...
  test("workspace runtime keeps tile, custom leaf, and command wiring alive", async () => {
    test.setTimeout(E2E_TIMEOUTS.extendedScenario);

    await test.step("workspace tile menu split and close still work after tile refactor", async () => {
    ...
    await test.step("custom leaf can still mount into the workspace mosaic", async () => {
    ...
    await test.step("workspace switch keeps plugins and runtime wiring alive", async () => {
      await openDocument(sharedPage, "Runtime Doc A");
