...
test.describe.serial("Offline-Created Document Sync", () => {
  ...
  test.beforeAll(async ({}, testInfo) => {
    ...
    await createDocument(sharedPage, "Offline Seed Doc");
  ...
  test("offline-created documents sync and survive reload after reconnect", async () => {
    test.setTimeout(E2E_TIMEOUTS.accountSetup);

    await test.step("create a document while offline and store it in offline-created", async () => {
    ...
    await test.step("sync automatically after reconnect without breaking the open editor", async () => {
    ...
    await test.step("synced offline-created documents survive reload and can be opened", async () => {
      await sharedPage.reload({ waitUntil: "domcontentloaded" });
      await waitForWorkspaceReady(sharedPage);
