...
async function createEditShareLinkFromUi(page: Page, title: string): Promise<string> {
  ...
  await expect(dialog.getByText("Share Access")).toBeVisible({ timeout: 10_000 });
...
async function typeInVisibleEditor(page: Page, text: string): Promise<void> {
  const codeMirror = page.locator('.cm-content[contenteditable="true"]').first();
...
test("anonymous edit share keeps syncing across burst edits and reload", async ({ browser }) => {
  ...
  try {
    ownerContext = await newE2EContext(browser, { bypassCSP: true, acceptDownloads: true });
    guestContext = await newE2EContext(browser, { bypassCSP: true, acceptDownloads: true });
    ...
    await expect(guestPage).toHaveURL(/\/share\/d\/[^/]+$/, { timeout: 60_000 });
...
test("logged-in browser can edit through a share link without breaking sync", async ({
  ...
  try {
    ownerContext = await newE2EContext(browser, { bypassCSP: true, acceptDownloads: true });
    recipientContext = await newE2EContext(browser, { bypassCSP: true, acceptDownloads: true });
    const ownerPage = await ownerContext.newPage();
    ...
    await expect(recipientPage).toHaveURL(/\/share\/d\/[^/]+$/, { timeout: 60_000 });
