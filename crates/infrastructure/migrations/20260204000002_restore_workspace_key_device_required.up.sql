-- Restore workspace_encrypted_keys to design spec where device_id is required
...
-- Step 4: Delete any records with NULL device_id (invalid data per design spec)
DELETE FROM workspace_encrypted_keys WHERE device_id IS NULL;

-- Step 5: Make device_id NOT NULL (as per design spec)
ALTER TABLE workspace_encrypted_keys
    ALTER COLUMN device_id SET NOT NULL;

-- Step 6: Make sender_device_id NOT NULL (as per design spec)
ALTER TABLE workspace_encrypted_keys
...
-- Step 8: Create primary key per design spec: (workspace_id, user_id, device_id, key_version)
ALTER TABLE workspace_encrypted_keys
